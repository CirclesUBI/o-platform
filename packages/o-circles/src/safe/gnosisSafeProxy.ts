...
export class GnosisSafeProxy extends Web3Contract {
  ...
  async execTransaction(
    ...
    const signedTransactionData = await this.execTransactionTxData(privateKey, safeTransaction, dontEstimate);
    ...
    await this.web3.eth
      ...
      .catch((e) => {
        ...
        if (e.data && e.data.startsWith("Reverted 0x") /* && e.data.length == 211*/) {
          ...
          const gasEstimateDataBuffer = Buffer.from(e.data.toString().substring(8));
