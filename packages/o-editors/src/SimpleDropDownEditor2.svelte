...
<div class="flex flex-row" style="height: 60vh">
  <div class="self-end w-full themed">
    <Select loadOptions="{loadOptions}" listOpen="{true}" itemId="{itemId}" floatingConfig="{floatingConfig}" listPlacement="top">
      <div class="h-screen beer" slot="item" let:item let:index>
        <BeerItem item="{item}" />
      </div>
