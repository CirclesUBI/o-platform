...
onMount(async () => {
  ...
  const safeBalanceAmount = Number.parseFloat(Web3.utils.fromWei(new BN(safeBalance).toString(), "ether")).toFixed(2);

  safexDai.balance = safeBalanceAmount;
  safexDai.title = $_("dapps.o-banking.pages.xDaiDetail.safe");
  safexDai.address = safeAddress;

  const eoaBalanceAmount = Number.parseFloat(Web3.utils.fromWei(new BN(eoaBalance).toString(), "ether")).toFixed(2);

  accountxDai.balance = eoaBalanceAmount;
  accountxDai.title = $_("dapps.o-banking.pages.xDaiDetail.safeOwner");
...
<div class="p-5">
  <div class="w-full mb-4 text-center">
    <h1 class="uppercase font-heading">{$_("dapps.o-banking.pages.xDaiDetail.xDai")}</h1>
  </div>
