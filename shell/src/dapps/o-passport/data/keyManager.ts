...
export class KeyManager {
  ...
  async keyFromPassphrase(passphrase: string) {
    ...
    return await crypto.subtle.importKey("raw", derivedKey, { name: "AES-CBC" }, false, ["encrypt", "decrypt"]);
  }
  ...
  async encryptWithPassphrase(passphrase: string, clearTextHexKey: string): Promise<EncryptedKey> {
    ...
    const cypherText = await crypto.subtle.encrypt({ name: "AES-CBC", iv }, subtleKey, keyBytes);
  ...
  async decryptWithPassphrase(
    ...
    const clearTextBytes = await crypto.subtle.decrypt({ name: "AES-CBC", iv: ivBytes }, subtleKey, base64CypherTextBytes);
