...
<div class="p-1 -mt-6 overflow-hidden text-white whitespace-pre-line xs:p-3 xs:-mt-2">
  ...
  <div class="text-center whitespace-pre-line">
    {#if !sessionStorage.getItem("inviteUrl")}
      ...
      <button class="relative px-8 mt-6 overflow-hidden text-lg transition-all transform btn btn-primary" on:click="{() => handleClick('back')}">
        {$_("dapps.o-homepage.components.survey.button.goBack")}</button>
    {:else}
      <button class="relative px-8 overflow-hidden text-lg transition-all transform btn btn-primary" on:click="{() => handleClick('submit')}" data-cy="finishSurvey">
