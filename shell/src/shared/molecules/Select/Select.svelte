...
<div
  ...
  use:floatingRef>
  {#if listOpen}
    <div use:floatingContent bind:this="{list}" class="svelte-select-list" class:prefloat="{prefloat}" on:scroll="{handleListScroll}" on:pointerup|preventDefault|stopPropagation>
      {#if $$slots["list-prepend"]}<slot name="list-prepend" />{/if}
      {#if $$slots.list}<slot name="list" filteredItems="{filteredItems}" />
  ...
  <div class="indicators">
    ...
    {#if showChevron}
      <div class="icon chevron" aria-hidden="true">
        <slot name="chevron-icon" listOpen="{listOpen}">
          <ChevronIcon />
        </slot>
      </div>
    {/if}
  ...
  <slot name="input-hidden" value="{value}">
    <input name="{name}" type="hidden" value="{value ? JSON.stringify(value) : null}" />
  </slot>

  {#if required && (!value || value.length === 0)}
    <slot name="required" value="{value}">
